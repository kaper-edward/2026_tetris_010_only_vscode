...
void SfmlRenderer::render(const RenderSnapshot& snapshot) {
    ...
    for (int y = 0; y < RENDER_BOARD_HEIGHT; ++y) {
        for (int x = 0; x < RENDER_BOARD_WIDTH; ++x) {
            ...
            cell.setFillColor(board_cell_color(board_cell(snapshot, y, x)));
            window_.draw(cell);
        }
    }
    ...
    sf::RectangleShape preview_shape({PREVIEW_CELL - 1.0f, PREVIEW_CELL - 1.0f});
    for (int y = 0; y < RENDER_BLOCK_SIZE; ++y) {
        for (int x = 0; x < RENDER_BLOCK_SIZE; ++x) {
            preview_shape.setPosition({SIDE_X + x * PREVIEW_CELL, OFFSET_Y + 145.0f + y * PREVIEW_CELL});
            if (preview_cell(snapshot.next_piece, y, x) == '#') {
                preview_shape.setFillColor(sf::Color(120, 210, 120));
            } else {
                preview_shape.setFillColor(sf::Color(30, 32, 36));
            }
            window_.draw(preview_shape);
        }
    }
    window_.display();
}
